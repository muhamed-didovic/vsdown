        ...
        return Promise
            ...
            .then(async (courses) => {
                ...
                return await Promise
                    .map(courses, async (url) => {//.slice(0, 2)
                        console.log('Parsing course with url:', url);
                        const lessons = await this.getSingleCourse({ p, browser, url, ms, ...opts });
                        if (!lessons.length) {
                            console.log(`Found not any videos for url: ${url}, download skipping... `);
                            return lessons;
                        }
                        console.log(`Found ${lessons[0].series} course that has ${lessons.length} lessons`);
            ...
            .then(async (lessons) => {
                ...
                return await Promise
                    .map(lessons, async (lesson, index) => {
                        ...
                        if (!vimeoData.length) {
                            //console.log(`no video or access for: ${lesson}`);
                            return;
                        }
                    ...
                    .then(async c => {
                        // console.log('1----------------------------------------------------------------', url, c[0].downPath);
                        // await browser.close()
        ...
        try {
            const v = await retry(async () => {//return
                ...
                if (body.includes('408 Request Time-out')) {
                    console.log('408 error:', vimeoUrl, body);
            ...
            // console.log('v', v, 'vimeoUrl',vimeoUrl);
            if (!v) {
                return {
                    size: -1,
                    url : vimeoUrl
    ...
    findVideoUrl(str, course) {
        ...
        if (res !== null && typeof res[0] !== "undefined") {
            try {
                ...
                console.log('error with findVideoUrl:', course.title, course.vimeoUrl, '-->err:', err);
        ...
        await Promise
            .all([
                ...
                (async () => {
                    ...
                    logger.succeed(`Downloaded course (total lessons: ${cnt})`)
                })()
            ])
    }
}
