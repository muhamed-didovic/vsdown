...
(async () => {
    ...
    if (flags.all || (input.length === 0 && await askOrExit({
        ...
        all({ email, password, logger, dir, concurrency, file, filePath, overwrite, markdown }).catch(errorHandler)
        return
    }
    ...
    one(courseUrl, { email, password, logger, dir, concurrency, overwrite, markdown }).catch(errorHandler)
})()
