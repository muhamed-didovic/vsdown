...
const newDownload = (url, dest, localSizeInBytes, remoteSizeInBytes, downFolder, index = 0, ms, resourceUrl) => {
    return new Promise(async (resolve, reject) => {
        ...
        await ytdown(url, dest, ms, index, localSizeInBytes, remoteSizeInBytes, logger, downFolder, resourceUrl, reject, resolve)
    });
};
 ...
 * @param ms
 */
export default async (file, dest, {overwrite, downFolder, index, resourceUrl} = {}) => {
    const url = file.url;
