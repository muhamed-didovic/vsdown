		...
		registerAction('afterResponse', async ({response}) => {
			...
			if (isHtml) {
				...
				async function hidePopup(page) {
					try {
					  // Wait for the element to be present first (timeout after 5 seconds)
					  await page.waitForSelector('#bb-banner', { timeout: 5e3 })
						.catch(() => console.log('Popup not found, continuing...'));
					  
					  // Hide the element if it exists
					  await page.evaluate(() => {
						const popup = document.querySelector('#bb-banner');
						if (popup) {
					  ...
					  if (!isHidden) {
						console.log('Warning: Popup might still be visible');
					  }
				...
				const elementExists2 = await page.$('#bb-banner') !== null//.onesignal-slidedown-dialog
                // logger.log('elementExists', elementExists, opts)
                if (elementExists2) {
                    logger.info('[PuppeteerPlugin] found popup bb-background');//onesignal-slidedown-dialog
                    // await page.click("#onesignal-slidedown-cancel-button");
                    //remove element from the page
                    await page.evaluate(selector => {
                        // This function will be executed within the page.
                    ...
                    }, '#bb-banner');//.onesignal-slidedown-dialog
                    await sleep(1e3);
                }
