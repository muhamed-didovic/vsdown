...
const ytdown = async (url, dest, ms, index, localSizeInBytes, remoteSizeInBytes, logger, downFolder, resourceUrl, reject, resolve) => {
    return await retry(async () => {//return
        ytDlpWrap
            ...
            .on("close", () => {
                ...
                // ms.remove(dest);
                logger.log(`${index}. End download ytdl: ${downFolder} Found url:${resourceUrl}`.green);
